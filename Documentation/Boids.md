...
-   **Reproduction and Nesting ([`js/main.js`](../js/main.js)):** The bird reproduction cycle is a multi-step process managed in the main simulation loop:
    1.  A bird first checks if the world population is below the configured maximum bird population. If not, it returns to hunting, incurring a "cost" by resetting its `beesCaught` counter.
